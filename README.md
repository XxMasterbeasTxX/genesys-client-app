...
| **Recording Playback** | Inline audio playback directly in the Interaction Detail drill-down panel. A "🎧 Play Recording" button fetches recordings on demand via a two-step API pattern (list stubs → per-ID presigned URL). Multiple recording segments (e.g. IVR leg + agent leg) are labelled Part 1, Part 2, etc. Screen recordings render as `<video>`. Handles archived, processing, and unavailable recordings gracefully. |
