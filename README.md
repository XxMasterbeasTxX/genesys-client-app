...
| **Agent Copilot Checklists** | Historical view of interactions that used Agent Copilot checklists. Cascading multi-select filters (Copilot → Queue → Agent), configurable period selector with 31-day API limit validation, background enrichment with abort-on-re-search. Drill-down shows individual checklist items with separate **Agent** and **AI** tick indicators — green ✓ / red ✗ colour-coded for instant readability. Status filtering (All / Completed / Incomplete / Summaries) — "All" shows every retrieved interaction, "Summaries" filters to interactions with AI-generated summaries. **Agent Checked** independent toggle filter — shows only interactions where the agent manually ticked at least one item; combinable with any status filter (e.g. Incomplete + Agent Checked). Search results table is collapsible via a toggle header — collapses automatically when an interaction is selected, re-expands when the detail panel is closed. |
| **Conversation Summaries** | Drill-down panel also displays AI-generated conversation summaries (headline, reason, resolution, followup, full text). Supports multiple summaries per conversation (e.g. transferred calls). Summaries are fetched alongside checklists during enrichment and rendered below the checklist items. Dedicated "Summaries" filter button to quickly find interactions with summaries. Dynamic extra topics — any additional AI-detected topics are auto-rendered without code changes, including their description and outcome when available. Agent-edited fields are highlighted with a ✏️ Edited badge; the original AI value is shown with strikethrough for comparison. |
| **Recording Playback** | Inline audio playback directly in the Interaction Detail drill-down panel. A **"🎧 Load Recordings"** button fetches recording stubs on demand (with an automatic one-time 3-second retry if Genesys hasn't indexed the recording yet). If no recordings exist, a message is shown. If recordings are found, one labelled button appears per segment (**Part 1**, **Part 2**, etc. for multi-part; **"🎧 Play Recording"** for single). Each button lazily fetches and shows its own audio player on first click; subsequent clicks toggle the player on/off. Screen recordings render as `<video>`. Handles archived, processing, and unavailable recordings gracefully. |
...
| `js/pages/dashboards/agent-copilot/checklistConfig.js` | Feature | Date range presets, query page size, enrichment batch size, API constants, chart appearance, export settings, UI labels (including Agent Checked button label) |
